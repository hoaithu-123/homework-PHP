    ...
    <div class="permissions">
        ...
        <div class="mt-8 bg-white rounded border-b-4 border-gray-300">
            <div class="flex flex-wrap items-center uppercase text-sm font-semibold bg-gray-300 text-gray-600 rounded-tl rounded-tr">
                <div class="w-5/12 px-4 py-3">Permission</div>
                <div class="w-5/12 px-4 py-3 ">Roles</div>
                <div class="w-2/12 px-4 py-3 text-right">Edit</div>
            </div>
            @foreach ($permissions as $permission)
                <div class="flex flex-wrap items-center text-gray-700 border-t-2 border-l-4 border-r-4 border-gray-300">
                    <div class="w-5/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $permission->name }}</div>
                    <div class="w-5/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">
                        @foreach($permission->roles as $role)
                            <span class="bg-gray-200 text-sm mr-1 mb-1 px-2 border rounded-full">{{ $role->name }}</span>
                        @endforeach
                    </div>
